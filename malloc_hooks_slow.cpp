...
std::string my_malloc_header::as_string() const
{
  size_t i = 0;
  for (void *const * p = &bt[0]; *p && (p < &bt[my_BT_DEEP]); ++p, ++i);
...
{
  return true;//magic == MAGIC_64BIT;
}

static std::map<void*, my_malloc_header> * allocated_pointers = NULL;
...
//  std::set<std::string> consumed_bts;
  ...
  for (std::map<void*, my_malloc_header>::const_iterator it = allocated_pointers->begin(); it != allocated_pointers->end(); ++it)
  {
    uint64_t cursize = it->second.size, cnt = 1;
    //NOTE START comment this code if you need for eventually allocated memory dump too
    if ((time_of_start - it->second.tstamp) != 0)
    {
      fprintf(f, "%p:%d\n", (void*)it->first, (int)cursize);
    }
    else 
      continue;
    //NOTE END
    std::string curbt = it->second.as_string();
//
//    int curaddrdiff = (char*)*it - curaddr;
    curaddr = (char*)it->first;
    curaddr -= ((long long unsigned int)curaddr) % my_PAGESIZE;
//    fprintf(stderr, "\taddr %p (diff %d)\n", (void*)curaddr, curaddrdiff);
    while (curaddr < (char*)it->first + it->second.size)
    {
...
//    consumed_bts.insert(curbt);
    std::map<void*, my_malloc_header>::const_iterator it_next = it;
    ++it_next;
    time_t tstart, tend;
    tstart = tend = it->second.tstamp;
    for (; it_next != allocated_pointers->end(); ++it_next)
    {
      if (it_next->second.as_string() == curbt)
      {
        cursize += it_next->second.size; // неоптимально, не удаляем ноду
        ++cnt;
        if (it_next->second.tstamp > tend)
        {
          tend = it_next->second.tstamp;
        }
        if (it_next->second.tstamp < tstart)
        {
          tstart = it_next->second.tstamp;
        }
      }
    }
 ...
 //                                                  ^---------------^
  ...
  {
    fprintf(stderr, "result_map.size() == 0, exit\n");
    return 0;
  }
...
my_init_hook (void)
{
  ...
  allocated_pointers = new std::map<void*, my_malloc_header>();
  SAVE_OLD_HOOKS
  RESTORE_MY_HOOKS
}
...
void my_int_free(void *& ptr)
{
  if (ptr)
  {
    std::map<void*, my_malloc_header>::iterator it = allocated_pointers->find(ptr);
    if (allocated_pointers->end() != it)
    {
      global_mem_usage_mhooks -= it->second.size;
...
void my_int_alloc(void *& ptr, size_t size)
{
  if (ptr && size)
  {
    ...
    (*allocated_pointers)[ptr].fill(size);
  }
}
